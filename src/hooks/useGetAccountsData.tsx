...
export default function UseGetAccountsData() {
  ...
  // PROCESSED DATA
  // Chart Data
  const [chartDataset, setChartData] = useState<ChartDataset[]>([
  ...
  function processChartData() {
    ...
    const datasets = accountsData.map((group, groupIndex) => {
      return {
        ...
        backgroundColor: allAccountNames.map((_, acctIndex) =>
          generateHSLColor(groupIndex + acctIndex)
        ),
      };
    });
  ...
  function getAccountsData() {
    ...
    // API Request
    // Check if data is present (else set state to false to indicate no data is present)
